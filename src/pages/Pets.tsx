...
import { Typography, Alert, Card, Tag, Empty } from 'antd'
...
function PetCard({ pet, cc }: { pet: PetDetail; cc: string }) {
  ...
  return (
    <Card
      ...
      hoverable
      className="pet-card"
      style={{ ['--cc']: cc, cursor: 'pointer' } as CSSProperties}
      styles={{ body: { padding: 14 } }}
      onClick={() => open(pet.name)}
      cover={
        <div className="pet-sprite-wrap" style={{
          ...
          {pet.sprite
            ? <LazyImage src={imgUrl('petsprite', pet.sprite)} alt={pet.name} preview={false} minHeight={120}
                imgStyle={{ maxHeight: 130, width: 'auto', objectFit: 'contain' }} />
    ...
    >
      ...
      {pet.skills.length > 0 ? pet.skills.slice(0, 3).map((sk, j) => (
        <div key={j} style={{ fontSize: 12.5, marginBottom: 4, lineHeight: 1.55, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          <Text strong>{sk.name}</Text>
          {sk.desc && <Text type="secondary">：{sk.desc}</Text>}
        </div>
      )) : <Text type="secondary" style={{ fontSize: 12.5 }}>技能未收录</Text>}
      <div style={{ marginTop: 8, fontSize: 12, color: cc, fontWeight: 600 }}>查看详情 →</div>
    </Card>
  )
}
  ...
  return (
    <div>
      ...
      <Alert type="info" showIcon style={{ marginBottom: 14 }}
        message="一宠一卡：点击卡片查看完整技能详情。整理自《口袋百科重制版·宠物篇》。" />
